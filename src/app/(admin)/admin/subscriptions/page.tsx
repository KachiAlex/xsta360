    ...
    <div className="space-y-5">
      <div>
        ...
        <p className="text-sm text-ink-soft m-0">
          {subs.length} active subscriptions · {orgsWithoutSubs.length} orgs on Free
        </p>
      </div>
            ...
            {subs.length === 0 ? (
              <tr>
                <td colSpan={6} className="px-4 py-8 text-center text-ink-soft">
                  No subscriptions yet. Assign plans to orgs from the Organizations page.
                </td>
              </tr>
            ) : (
              subs.map((s) => (
        ...
        {subs.length === 0 ? (
          ...
          subs.map((s) => (
            <a key={s.id} href={`/admin/orgs/${s.orgId}`} className="block px-4 py-3 active:bg-paper-2/30">
              ...
              <div className="text-xs text-ink-soft">{s.planName}</div>
            </a>
          ))
        )}
      </div>
        ...
        <div className="bg-panel border border-rule rounded-md">
          <div className="px-4 py-3 border-b border-rule">
            <h2 className="font-mono text-sm uppercase tracking-wider m-0">
              On Free (no subscription) — {orgsWithoutSubs.length}
            </h2>
          </div>
